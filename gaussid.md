...
$$S = \frac{1}{2} \log_2 [(2\pi e)|\Sigma|] + \text{const bits}$$
...
$$x\sim\mathcal{N}(\mu,\Sigma)\Rightarrow(x-\mu)^{T}\Sigma^{-1}(x-\mu)\sim\chi_{n}^{2}$$
...
$$\mathcal{N}(a,A)*\mathcal{N}(b,B)\propto\mathcal{N}(a+b,A+B)$$
...
subject to the $J$ conditions $c_{j}(x)=0$ is given by:

$$A\mu+AC\Lambda$$

$$\Lambda=-4(C^{T}AC)C^{T}A\mu$$
